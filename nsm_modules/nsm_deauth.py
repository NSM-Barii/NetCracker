...
class Frame_Snatcher:
        ...
        try:
            ...
            while True:
                ...
                if iface == "" and def_iface == "":
                    console.print(
                        "You must enter iface to procced silly", style="bold red"
                    )
    ...
    def sniff_for_targets(cls, iface="wlan0", verbose=1, timeout=15):
        ...
        try:
            while True:
                ...
                sniff(
                    iface=iface,
                    prn=Frame_Snatcher.packet_parser,
                    count=0,
                    store=0,
                    timeout=15,
                )
    ...
    def packet_parser(cls, pkt):
        ...
        def parser(pkt):
            ...
            if pkt.haslayer(Dot11Beacon):
                ...
                frequency = Background_Threads.get_freq(
                ...
                if addr1 not in cls.macs and addr1 != "No":
                    ...
                    cls.beacons.append(
                        (ssid, addr1, vendor, encryption, frequency, channel, rssi)
                    )
                    ...
                    console.print(
                        f"[{c2}][+] Found MAC addr:[{c4}] {addr1}  -  {channel}"
                    )
                ...
                if addr2 not in cls.macs and addr2 != "No":
                    # ADD MAC
                    cls.beacons.append(
                        (ssid, addr2, vendor, encryption, frequency, channel, rssi)
                    )
                    ...
                    # NOW TO OUTPUT RESULTS
                    console.print(
                        f"[{c2}][+] Found MAC addr:[{c4}] {addr2}  -   {channel}"
                    )  #  -  [{c1}]{ssid}")
    ...
    def track_clients(cls, target, iface, track=True, delay=5):
        ...
        def sniff_for_clients(timeout=0):
            ...
            while cls.SNIFF:
                sniff(
                    iface=iface, prn=parse_for_clients, count=0, store=0, timeout=2
                )  # timeout=timeout)
    ...
    def target_chooser(cls, type):
        ...
        def rescan_option():
            ...
            # USER INPUT
            choice = (
                console.input("[bold red]Do you want to rescan?[bold green] (y/n): ")
                .strip()
                .lower()
            )
    ...
    def client_chooser(cls, target, iface, verbose=0, timeout=120):
        ...
        with Live(table, console=console, refresh_per_second=2):
            sniff(
                iface=cls.iface, prn=client_sniffer, store=0, count=0, timeout=timeout
            )
    ...
    def target_attacker(
        ...
        threading.Thread(
            target=Frame_Snatcher.track_clients, args=(target, cls.iface), daemon=True
        ).start()
    ...
    def main(cls, type, skip=False):
        ...
        try:
            ...
            # SINGLE CLIENT
                ...
                while cls.GO:
                    Frame_Snatcher.target_attacker(
                        target=target, client=client, iface=cls.iface
                    )
                    Background_Threads.channel_hopper(set_channel=channel)
