...
class ReconScanner:
        ...
        try:
            ...
            for net in networks:
                ...
                if net.bssid not in cls.networks:
                    ...
                    # NOW TO SAVE DATA
                    cls.network_saver.network_logging(
                        ssid=net.ssid,
                        bssid=net.bssid,
                        signal=net.signal,
                        frequency=frequency,
    ...
    def loop_controller(cls, interval=3):
        ...
        if NetTilities.get_iface(verbose=True):
            ...
            table_results = Table(
                title="Recon Mode", style="bold purple", header_style="bold red"
            )
            ...
            table_results.add_column(
                "Encryption"
            )  # AKA AKM // AUTHENCTION AND KEY MANAGEMENT
            table_results.add_column("Channel")
            table_results.add_column("BSSID", style="cyan")

            # INDEFINITE UNTIL - CTRL + C
            with Live(table_results, console=console, refresh_per_second=0.5):
                while True:
                    try:
